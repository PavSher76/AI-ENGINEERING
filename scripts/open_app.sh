...
# Проверяем Keycloak
if curl -k -s -o /dev/null -w "%{http_code}" "https://localhost:8080" | grep -q "200\|302"; then
...
echo
if [[ $REPLY =~ ^[Yy]$ ]]; then
    echo "🔑 Открытие Keycloak Admin: https://localhost:8080/admin"
    open "https://localhost:8080/admin" 2>/dev/null || {
        echo "   Не удалось открыть браузер автоматически"
        echo "   Откройте вручную: https://localhost:8080/admin"
    }
fi
